...
(function ($) {
    ...
    function CompositeEditor(columns, containers, options) {
        ...
        function editor(args) {
            ...
            function init() {
                var newArgs = {};
                var columnCount = columns.map(function(val) {
                    return val.compositeEditor || val. editor ? 1 : 0;
                }).reduce(function(prev, curr) {
                    return prev + curr;
                });
                var idx = columnCount;
                editors = new Array(columnCount);
                while (idx--) {
                    var editor = columns[idx].compositeEditor || columns[idx]. editor;
                    if (editor ) {
                        newArgs = $.extend({}, args);
                        ...
                        editors[idx] = new (editor)(newArgs);
                    }
                }
            }
